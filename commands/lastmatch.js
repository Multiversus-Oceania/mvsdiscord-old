    ...
    async execute(interaction) {
        ...
        const embed = new Discord.EmbedBuilder()
            ...
            .addFields(
                ...
                { name: "Blue Team", value: teamPlayers[0].map(p => `**${p.playerName}** ${p.character}\nPlayer Rating: ${parseInt(p.playerRating, 10)}${p.playerRatingChange > 0 ? " (+" : p.playerRatingChange < 0 ? " (-" : ""}${Math.abs(p.playerRatingChange)})\nCharacter Rating: ${parseInt(p.characterRating, 10)}${p.ratingChange > 0 ? " (+" : p.ratingChange < 0 ? " (-" : ""}${Math.abs(p.ratingChange)})\nDamage Done: ${p.damageDone}\nRingouts: ${p.ringouts}\nDeaths: ${p.deaths}`).join("\n\n"), inline: true, color: 0x0000ff },
